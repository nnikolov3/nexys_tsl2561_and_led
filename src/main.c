...
void Parse_Input_Task( void* p )
{
    ...
    while( 1 )
    {
        ...
        {
            uint8_t btns =
              ( NX4IO_getBtns( ) & 0x0C ) >> 2; // Get BTNU/BTND, right justify
            uint8_t sws = (uint8_t) ( NX4IO_getSwitches( ) &
                                      0x00FF ); // Get lower 8 switches
            ValueToSend =
              ( ( btns << 8 ) |
                sws ); // Combine: btns in upper 8 bits, sws in lower
...
void PID_Task( void* p )
{
    ...
    while( 1 )
    {
        ...
        NX4IO_setLEDs( btnSws ); // Update regular LEDs with button/switch state
