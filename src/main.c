...
void Parse_Input_Task( void* p )
{
        ...
        while( 1 )
        {
                ...
                if( xSemaphoreTake( binary_sem, pdMS_TO_TICKS( 500 ) ) ==
                    pdTRUE )
                {
                        uint8_t btns = ( NX4IO_getBtns( ) & 0x0C ) >>
                                       2; // Get BTNU/BTND, right justify
                        uint8_t sws =
                          (uint8_t) ( NX4IO_getSwitches( ) &
                                      0x00FF ); // Get lower 8 switches
                        ValueToSend =
                          ( ( btns << 8 ) | sws ); // Combine: btns in upper 8
                                                   // bits, sws in lower
...
void PID_Task( void* p )
{
        ...
        while( 1 )
        {
                ...
                /* Get TSL2561 reading */
                uint16_t ch0 =
                  tsl2561_readChannel( &IicInstance, TSL2561_CHANNEL_0 );
                uint16_t ch1 = tsl2561_readChannel(
                  &IicInstance, TSL2561_CHANNEL_1 ); // Now used
                tsl2561 = (float) ch0 -
                          ( (float) ch1 *
                            0.5 ); // Basic lux approximation using CH0 and CH1
                ...
                if( pidOUT >= 0 )
                {
                        if( ( pwmLED + (uint8_t) ( pidOUT * MAX_DUTY ) ) >=
                            MAX_DUTY )
                        {
                                pwmLED = MAX_DUTY;
                        }
...
int pid_init( PID_t* pid )
{
        ...
        pid->prev_error = 0;
        pid->delta_t =
          0.437; // Initial delta_t (437ms, worst-case sampling time)
